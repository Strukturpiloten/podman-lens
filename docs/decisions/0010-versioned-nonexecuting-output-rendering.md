...
containers and records infra-container mounts for pods, plus
...
M6-B4 replaces the narrow mount and secret-prerequisite APIs directly. Container `MountIntent`
has named-volume, bind, and tmpfs forms; volume subpath is exact only with normal copy behavior.
Pod infra mounts deliberately block all rendering because native `pod create` has no matching CLI
mount flags even though its API accepts related fields. `VolumeIntent` retains independently
optional UID/GID and renders them only from Podman 5.6. `SecretGrant` has mounted and environment
forms, with no secret bytes in the plan or artifacts. `ImageIntent` requires explicit policy and
source classification: policy has exact CLI/API evidence only from 5.6, while valid local,
unqualified, and tagless spellings produce `PLN0048` manual portability findings without rewriting
the source or inserting an implicit pull.
...
- Later M6-B work adds remaining native settings beyond the bounded typed mount, secret, image,
  networking, and runtime surfaces.
