...
if ((${#missing_tools[@]} != 0)); then
  ...
  printf 'PodmanLens file checks are missing required tool(s):%s. Use the Dev Container.\n' \
    "${missing_list}" >&2
  exit 2
fi
...
mapfile -d '' structured_files < <(list_existing_files '*.json' '*.jsonc' '*.yaml' '*.yml' '*.code-workspace')
...
printf '\nPodmanLens non-Rust file checks passed in %s mode.\n' "${mode#--}"
