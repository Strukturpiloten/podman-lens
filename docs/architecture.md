...
The boundary is recorded in [ADR 0016](decisions/0016-captured-native-release-evidence.md). A
separate reusable release worker validates the exact checkout through the production read-only
acquisition path against a fresh, isolated Podman service; compact evidence binds candidate SHA,
run ID, run attempt, and task. It cannot discover an ambient socket or retain a native payload. See also
[ADR 0017](decisions/0017-isolated-native-release-conformance.md).
