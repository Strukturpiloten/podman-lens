...
REPOSITORY_POLICY = planner.policy_at(SOURCE_ROOT)
# Keep the shared classifier's prose-versus-executable regression fixtures
# independent of PodmanLens's stricter all-public-guides policy.
POLICY = copy.deepcopy(REPOSITORY_POLICY)
POLICY["executable_documentation_roots"] = []
...
class ValidationPlanTests(unittest.TestCase):
    ...
    def test_plain_podman_public_guide_requires_inventory_contract(self) -> None:
        write(self.root, "docs/public/concepts/index.md", "# Concepts\n\nPlain wording changed.\n")
        head = self.commit("plain public guide")
        plan = planner.make_plan(arguments(self.root, "pull_request", self.base, head), REPOSITORY_POLICY)
        self.assertEqual(plan["profile"], "executable-docs")
        self.assertIn(
            ["cargo", "test", "--locked", "--test", "public_guides"],
            REPOSITORY_POLICY["local_commands"]["executable-docs"],
        )
        self.assertNotIn(
            ["cargo", "test", "--locked", "--test", "public_guides"],
            REPOSITORY_POLICY["local_commands"]["prose"],
        )
    ...
    def test_mixed_space_tab_indented_code_is_executable(self) -> None:
        write(self.root, "docs/public/concepts/index.md", "# Concepts\n\n  \tcargo test\n")
        head = self.commit("mixed indentation")
        self.assertEqual(self.plan(self.base, head)["profile"], "executable-docs")
