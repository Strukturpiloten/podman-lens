...
struct ResourceDetailsSnapshot {
    ...
    image_repo_tags: Option<FieldCountSnapshot>,
    image_repo_digests: Option<FieldCountSnapshot>,
    image_digest: Option<FieldStateSnapshot>,
    image_created: Option<FieldStateSnapshot>,
    image_author: Option<FieldStateSnapshot>,
    image_architecture: Option<FieldStateSnapshot>,
    image_operating_system: Option<FieldStateSnapshot>,
    image_manifest_type: Option<FieldStateSnapshot>,
    ...
    secret_driver: Option<NativeSecretDriverSnapshot>,
    secret_created_at: Option<FieldStateSnapshot>,
    secret_updated_at: Option<FieldStateSnapshot>,
    volume_uid: Option<VolumeOwnerFieldSnapshot>,
    volume_gid: Option<VolumeOwnerFieldSnapshot>,
    volume_driver: Option<FieldStateSnapshot>,
    volume_created_at: Option<FieldStateSnapshot>,
    volume_anonymous: Option<FieldStateSnapshot>,
...
/// Redacted state/count-only native secret-driver snapshot. Option names and values are omitted.
#[derive(Debug, Serialize)]
struct NativeSecretDriverSnapshot {
    #[serde(flatten)]
    field: FieldStateSnapshot,
    name: FieldStateSnapshot,
    options: FieldCountSnapshot,
...
fn details(source: &ResourceDetails) -> ResourceDetailsSnapshot {
    match source {
        ResourceDetails::Container(value) => ResourceDetailsSnapshot {
            ...
            image_repo_tags: None,
            image_repo_digests: None,
            image_digest: None,
            image_created: None,
            image_author: None,
            image_architecture: None,
            image_operating_system: None,
            image_manifest_type: None,
            network: None,
            ...
            volume_gid: None,
            volume_driver: None,
            volume_created_at: None,
            volume_anonymous: None,
        },
        ...
        ResourceDetails::Volume(value) => ResourceDetailsSnapshot {
            ...
            volume_driver: Some(field_summary(value.driver())),
            volume_created_at: Some(field_summary(value.created_at())),
            volume_anonymous: Some(field_summary(value.anonymous())),
        },
        ResourceDetails::Image(value) => ResourceDetailsSnapshot {
            ...
            mounts: None,
            secret_grants: None,
            image_repo_tags: Some(collection_summary(value.repo_tags(), Vec::len)),
            image_repo_digests: Some(collection_summary(value.repo_digests(), Vec::len)),
            image_digest: Some(field_summary(value.digest())),
            image_created: Some(field_summary(value.created())),
            image_author: Some(field_summary(value.author())),
            image_architecture: Some(field_summary(value.architecture())),
            image_operating_system: Some(field_summary(value.operating_system())),
            image_manifest_type: Some(field_summary(value.manifest_type())),
            network: None,
            networking: None,
            memory_swappiness: None,
            infra: None,
        ...
        ResourceDetails::Secret(value) => ResourceDetailsSnapshot {
            ...
            resource_controls: None,
            create_infra: None,
            secret_driver: Some(native_secret_driver(value.driver())),
            secret_created_at: Some(field_summary(value.created_at())),
            secret_updated_at: Some(field_summary(value.updated_at())),
            volume_uid: None,
            volume_gid: None,
            volume_driver: None,
            volume_created_at: None,
...
fn native_secret_driver(value: &ObservationField<crate::NativeSecretDriverObservation>) -> NativeSecretDriverSnapshot {
    let field = field_summary(value);
    let Some(value) = value.observed().map(crate::ObservedValue::value) else {
        return NativeSecretDriverSnapshot {
            field: field.clone(),
            name: field.clone(),
            options: empty_collection_summary(&field),
        };
    };
    NativeSecretDriverSnapshot {
        field,
        name: field_summary(value.name()),
        options: collection_summary(value.options(), crate::NativeSecretDriverOptions::len),
    }
}
