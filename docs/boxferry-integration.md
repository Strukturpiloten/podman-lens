...
| `secret_grants`                                                 | `Service::secret_grants` / `ResourceGrant`                             | Inspect retains reference and effective UID/GID/mode evidence, but not delivery form or target. An adapter must not construct a grant until the caller separately authors form and target; secret bytes remain absent. |
