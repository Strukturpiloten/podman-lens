...
case "$(uname -m)" in
  x86_64)
    readonly release_architecture="x86_64" shfmt_architecture="amd64" tombi_architecture="x86_64-unknown-linux-musl"
    ...
    ;;
  aarch64 | arm64)
    readonly release_architecture="aarch64" shfmt_architecture="arm64" tombi_architecture="aarch64-unknown-linux-musl"
...
download() {
  ...
  curl --proto '=https' --tlsv1.2 --fail --location --silent --show-error --output "${destination}" "${url}"
  printf '%s  %s\n' "${checksum}" "${destination}" | sha256sum --check --status
}
install -d "${install_directory}"

tombi_archive="${temporary_directory}/tombi.tar.gz"
download "https://github.com/tombi-toml/tombi/releases/download/v${tombi_version}/tombi-cli-${tombi_version}-${tombi_architecture}.tar.gz" "${tombi_archive}" "${tombi_checksum}"
tar --extract --gzip --file "${tombi_archive}" --directory "${temporary_directory}"
install -m 0755 "${temporary_directory}/tombi-cli-${tombi_version}-${tombi_architecture}/tombi" "${install_directory}/tombi"

shfmt_binary="${temporary_directory}/shfmt"
download "https://github.com/mvdan/sh/releases/download/v${shfmt_version}/shfmt_v${shfmt_version}_linux_${shfmt_architecture}" "${shfmt_binary}" "${shfmt_checksum}"
install -m 0755 "${shfmt_binary}" "${install_directory}/shfmt"

shellcheck_archive="${temporary_directory}/shellcheck.tar.xz"
download "https://github.com/koalaman/shellcheck/releases/download/v${shellcheck_version}/shellcheck-v${shellcheck_version}.linux.${release_architecture}.tar.xz" "${shellcheck_archive}" "${shellcheck_checksum}"
tar --extract --xz --file "${shellcheck_archive}" --directory "${temporary_directory}"
install -m 0755 "${temporary_directory}/shellcheck-v${shellcheck_version}/shellcheck" "${install_directory}/shellcheck"

hadolint_binary="${temporary_directory}/hadolint"
download "https://github.com/hadolint/hadolint/releases/download/v${hadolint_version}/hadolint-linux-${release_architecture}" "${hadolint_binary}" "${hadolint_checksum}"
